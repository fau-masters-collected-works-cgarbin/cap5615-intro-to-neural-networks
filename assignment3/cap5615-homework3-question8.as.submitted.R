...
gradient_descent <- function(training_data, eta, error_threshold, niter) {
  ...
  while (current_error > error_threshold & k <= niter) {
    ...
    for (instance in 1:nrow(training_data)) {
      # Save these to avoid multiple conversions and run a bit faster
      features_numeric <- as.numeric(features[instance, ])
      label_numeric <- as.numeric(labels[instance]) # d(n)
...
test_result <- gradient_descent(randomized_test.set, learning_rate, error_threshold, iterations)
plot(1:iterations, train_result$e, col = "red", ylim = c(0, 100))
par(new = TRUE)
plot(1:iterations, test_result$e,
  col = "blue", ylim = c(0, 100),
  xlab = "Iteration number", ylab = "Error count"
)
